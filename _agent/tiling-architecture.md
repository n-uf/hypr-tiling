...
- The built-in group tab strip (`react/group-tab-strip.tsx`, gated by `grouping.showGroupTabStrip`, default on) is the canonical group chrome. `grouping.groupTabStrip` resolves placement (`top` above the pane header, `bottom` under the body), height (default 28px, subtracted from the member content box in both placements), eject/ungroup flags, and CSS theme tokens applied as inline styles and `--hpt-group-tab-*` custom properties the same way `dragChrome` resolves partial tokens. A drop on a tab measures `[role="tab"]` bounds (`measureGroupTabMemberRects`) and sets `memberInsertIndex` so `addLeafToGroup` splices at that slot; host drop targets omit the index and still append.
