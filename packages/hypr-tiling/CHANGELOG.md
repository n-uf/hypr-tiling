...
- Home: the hero slot is a tab group, hypr-tiling | Proof | Scenarios, with the hero active on first paint; the four original tiles keep their places and widths. Proof facts (suite and test counts, gzip sizes, dependencies, license, version) are generated from the package. Scenarios run through the public command dispatch and the workspace-set controller.
