...
- **Set mode takes the whole tile pool in `tiles`** — coverage is now checked **per tree**: the active workspace's layout is healed against the tiles *it* seats (no empty leaves, no duplicate seats, no seat for a tile missing from the pool), never against every pool tile. Pre-H6 the wrapper reused the single-layout heal, so a pool tile shown only in another workspace was force-seated into the active tree on the next integrity settle and a seat for an unknown tile was pruned. Single-layout mode is byte-identical (the heal still covers the whole `tiles` map). Set-wide integrity (`workspaceSetIssues`) moved to the wrapper.
- **`orphanTiles?: "seat-in-active" | "ignore" | "report"`** (set mode, default `"report"`) — what the wrapper does with a pool tile no workspace seats and a seat whose tile is not in the pool. `"report"` leaves the set untouched and calls `onIntegrityIssues`; `"seat-in-active"` runs `repairWorkspaceSet` (orphans seated `TILING_DEFAULT_WORKSPACE_PLACEMENT` in the active tree, unknown seats pruned) once per set identity and emits the repaired set on `onWorkspacesChange`, then reports the issues it repaired; `"ignore"` does neither.
- **`mintLeafId?: (tileId) => string`** (set mode) — leaf id for a seat minted by `"seat-in-active"`; default `` `leaf:${tileId}` ``.
- **`onIntegrityIssues?: (issues: ReadonlyArray<TilingWorkspaceSetIssue>) => void`** (set mode) — fires when the issue **fingerprint** (`kind|workspaceId|leafId|tileId` rows) changes, so a host re-rendering with the same set + pool is not re-notified; an empty list fires once when the last issue clears. Not called under `"ignore"`.
- **`inactiveWorkspaces?: "unmount" | "keep-mounted"`** (set mode, default `"unmount"`) — `"keep-mounted"` keeps a tile-keyed stable pane (`paneIdentity: "stable"`) whose tile is seated **only** in an inactive workspace parked in the hidden pool (`display: none`) instead of unmounting it, so an iframe / video / editor survives the switch away and back. Costs: every retained pane's DOM and React subtree stays alive (memory + hidden-subtree effects); a pane is retained only after it has rendered once (never-shown tiles stay lazy); an **empty** active workspace still unmounts the tree + pool; a no-op under `paneIdentity: "slot"`.
- **`TilingRenderTileProps.workspaceId: string`** / **`seatCount: number`** — the workspace the pane is painted in and how many workspaces seat the tile. Single-layout mode reports `TILING_MAIN_WORKSPACE_ID` / `1`. A retained (`keep-mounted`) pane keeps the workspace id of its last live paint.
...
- `onWorkspaceSwitch?: (event: { from; to; via: "tab" | "key" | "command" | "swipe" | "spring-load" | "reveal" })` — fired beside `onWorkspacesChange` when `activeId` changes through the renderer (`"swipe"` and `"spring-load"` are emitted by the bullets below; `"tab"` reserved, not emitted yet).
...
- **`WorkspaceTransitionStage`** / **`useWorkspaceTransition`** — self-contained stage + imperative `begin({ direction, mode })` / `scrub(progress)` / `finish("commit" | "cancel")`. Progress-driven while N1 swipe `progress` is non-null; timed curve when `finish` runs. A second `begin` mid-flight replaces the clone.
...
- **`interaction.workspaces.switch.transition?: "none" | "slide" | "fade"`** (default `"none"`; resolved on `ResolvedTilingWorkspaceSwitchCapability.transition`) — the set-mode wrapper now mounts `WorkspaceTransitionStage` inside the viewport around the live tree when the mode is not `"none"` and drives it itself: `begin` runs **before** `onWorkspacesChange` for every renderer-driven switch (command, key, tab, reveal, swipe) and, for a host-driven `activeId` change, in a pre-mutation snapshot, so the frozen clone always shows the outgoing workspace; `finish("commit")` runs once the incoming tree is live. A tracking swipe scrubs the stage from its `progress`; a swipe released short of the commit threshold settles it back with `finish("cancel")`; a committed swipe keeps its capture (no second `begin`). `cycle-workspace` keeps the cycle's own direction across a wrap-around; any other switch derives `prev` / `next` from tab order. Reduced motion (`prefers-reduced-motion: reduce`) and a degraded capture resolve through `resolveTransitionMode` as before. The stage lives inside the existing viewport (`position: relative; overflow: hidden`); the swipe root's `overscroll-behavior-x` is set once. Hosts that drove `useWorkspaceTransition` by hand should drop that and set the capability.
- `onWorkspaceSwitch({ via: "spring-load" })` is emitted for a spring-loaded tab drop (see `interaction.workspaces.springLoad` below); `"tab"` stays reserved (a host tab strip switches through `switch-workspace`, reported as `"command"`).
...
- **`interaction.workspaces.switch?: { wheelSwipe?: boolean | Partial<TilingWorkspaceSwipeConfig>; touchSwipe?: boolean }`** — opt-in swipe navigation (both default `false`). Resolved as `ResolvedTilingWorkspaceSwitchCapability` (`{ wheelSwipe, touchSwipe, swipe: TilingWorkspaceSwipeConfig }`; `resolveWorkspaceSwitchCapability` on `./engine`). When enabled the set-mode root carries inline `overscroll-behavior-x: contain`, plus `touch-action: pan-y` for touch. A committed swipe dispatches `cycle-workspace` through the renderer's command path and fires `onWorkspaceSwitch({ via: "swipe" })`.
- **Swipe settle waits for the switch transition** — with `switch.transition` on, the swipe FSM stays `settling` (and `useWorkspaceSwipe().phase` reads `"settling"`) until `WorkspaceTransitionStage` has slid to the incoming workspace (commit) or back to the outgoing one (cancel); `SETTLE_DONE` then moves it to `lockout` / `idle`. With `"none"` (the default), reduced motion or no mounted stage the settle stays immediate. Disabling the swipe input or unmounting mid-settle force-settles. Internal driver param `settleGate?: (outcome, done) => void` on `UseWorkspaceSwipeDriverParams` (the wrapper implements it over the stage's `onSettled`).
- **`useWorkspaceSwipe(): { progress, target, phase }`** + **`TilingWorkspaceSwipeScope`** — headless read of the gesture (`progress` `-1..1`, negative = previous). Live when the hook and the set-mode renderer share a scope; the idle snapshot otherwise. Types `TilingWorkspaceSwipeSnapshot`, `TilingWorkspaceSwipePhase`, `TilingWorkspaceSwipeTarget`; `TILING_WORKSPACE_SWIPE_DEFAULTS` on `.`.
- **`workspaceSwipeReducer(state, event, config?)`** (`./engine`) — pure swipe FSM `idle → armed → tracking → settling → lockout → idle` over `WHEEL` / `TOUCH_START` / `TOUCH_MOVE` / `TOUCH_END` / `DRAG_ACTIVE` / `IDLE_TICK` / `SETTLE_DONE` / `CANCEL` / `SET_CONTEXT`. Arms only when no drag is active, `Ctrl` is up, the scroll chain is exhausted, `|dx| > 2|dy|`, and a neighbour exists (or `wrap`). Commits when `|progress| ≥ commitFraction` or the peak velocity towards the target `≥ commitVelocityPxMs`; `settling(commit)` carries the `cycle-workspace` command. Helpers `resolveSwipeArming`, `swipeProgressOfTravel`, `shouldCommitSwipe`, `swipeCommitCommand`, `hasSwipeNeighbour`, `swipeTargetOfTravel`, `workspaceSwipeSnapshot`, `resolveWorkspaceSwipeConfig`; constants `TILING_WORKSPACE_SWIPE_INITIAL_STATE` / `_EMPTY_CONTEXT` / `_IDLE_SNAPSHOT`.
- **Ports** — framework-free `WheelTouchInputPort` / `WheelTouchInputListener` / `WheelInputSample` / `TouchInputSample` and `ScrollChainPort` (`./engine`); DOM adapters `createDomWheelTouchPort(element, { wheel, touch, scrollChain })` (wheel listener passive at rest, non-passive + `preventDefault` only while tracking; touch via primary `pointerType: "touch"` pointer events), `normaliseWheelDelta`, `createDomScrollChainPort(rootRef)`, `canElementScrollFurther` (`.`).
- **`springLoadReducer(state, event, config?)`** (`./engine`) — pure dwell FSM behind the spring-loaded workspace-tab drop (engine half of N3): `idle → dwelling → fired` over `HOVER { hover, leafId, activeWorkspaceId, ts }` / `TICK { ts }` / `DRAG_END` / `RESET`. A `kind: "workspace-tab"` hover over a non-active workspace starts the dwell, the same tab continues it, a different tab restarts `since`, `null` / a non-tab hover / the active workspace's own tab return to `idle`. `TICK` at `ts − since ≥ dwellMs` fires ONCE with `intent: { kind: "commit-and-rearm", leafId, workspaceId, placement }`; `fired` is absorbing until `RESET` / `DRAG_END`. The tree never changes under a live drag: the host ends the running drag as the existing `claimed` external commit (`moveLeafToWorkspace` + `switchWorkspace`) and re-arms a fresh drag on the leaf's new seat. Helpers `springLoadFireAt(state, config)` (the timer deadline), `isWorkspaceTabDragHover`; constants `TILING_SPRING_LOAD_INITIAL_STATE`; types `TilingSpringLoadState` / `TilingSpringLoadEvent` / `TilingSpringLoadIntent` / `TilingSpringLoadPhase`.
- **`TILING_SPRING_LOAD_DEFAULTS`** / **`TilingSpringLoadConfig`** — `{ dwellMs: 500 }`; `resolveSpringLoadConfig(partial?)` merges (clamped at `0`). **`TilingSpringLoadCapability`** — `Partial<TilingSpringLoadConfig> | false`, the value `interaction.workspaces.springLoad` takes; `resolveSpringLoadCapability(value)` → config or `null` (disabled).
- **`interaction.workspaces.springLoad?: TilingSpringLoadCapability | null`** (set mode, default off; resolved on `ResolvedTilingWorkspacesCapability.springLoad: TilingSpringLoadConfig | null`, `null` reads as `false`) — spring-loaded tab drop. While a pane drag reports a `kind: "workspace-tab"` external hover (`resolveExternalDragHover` or the `externalDragHover` prop) over a workspace that is not the active one, the wrapper runs `springLoadReducer` and arms one scheduler timer for the dwell (`dwellMs`, default 500 ms; the active workspace's own tab never dwells, leaving the tab before the deadline resets it, a new tab restarts it). When it fires the running drag ends as the existing claimed external commit and, in one `onWorkspacesChange` emission, the leaf is moved into the hovered workspace (`moveLeafToWorkspace`, default placement) and the set switches there (`switchWorkspace`, with `beginSwitchTransition` first so a `switch.transition` animates the switch); `onMoveLeaf(leafId, from, to)` and `onWorkspaceSwitch({ from, to, via: "spring-load" })` fire, and the leaf becomes the destination's remembered focused leaf. Pointer capture is kept and the drag continues on the leaf's seat in the destination tree without a new `pointerdown` (drag-machine `REARM` after the destination tree has rendered; the pickup tail — root capture + initial target — re-runs, so the gesture can keep dragging, spring-load again, drop or cancel as an ordinary drag). If the host renders a different workspace or a tree without the leaf instead, the rearm is dropped and the pointer released. The host's `onExternalDragHoverChange` keeps receiving every change. A release or Escape mid-dwell ends the dwell (the pre-N3 tab drop / cancel run as before).
- **Drag machine `REARM` event** (`DragRearmEvent`, `./engine`) — `{ type: "REARM", pointerId, pointerType, sourceLeafId, tileId?, anchorFootprint, pointerAnchorOffset?, client }` starts a NEW drag on a leaf at the current pointer point without a `pointerdown`: `idle → dragging` directly, `resolvedTarget: null`, every footprint from the event (the new tree) — no footprint of the previous drag survives, so a cancel after a rearm flies back to the leaf's new seat. Accepted ONLY in `idle` (`canRearmDrag(state)`); ignored in `armed` / `dragging` / `settling` so it can never fork a live drag or skip a settle teardown. Strip / watchdog / visibility recovery paths treat the re-armed drag as an ordinary drag (INV-R1..R4 unchanged). `rearmPointerAnchorOffset(footprint, client)` — the default grab offset: the pointer's own offset inside the footprint, else its centre (the pointer is typically still over the tab strip at rearm time).
