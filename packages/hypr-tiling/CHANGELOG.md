...
- `onWorkspaceSwitch?: (event: { from; to; via: "tab" | "key" | "command" | "swipe" | "spring-load" | "reveal" })` — fired beside `onWorkspacesChange` when `activeId` changes through the renderer (`"spring-load"` / `"tab"` reserved, not emitted yet).
...
- **`TILING_SPRING_LOAD_DEFAULTS`** / **`TilingSpringLoadConfig`** — `{ dwellMs: 500 }`; `resolveSpringLoadConfig(partial?)` merges (clamped at `0`). **`TilingSpringLoadCapability`** — `Partial<TilingSpringLoadConfig> | false`, the value the planned `interaction.workspaces.springLoad` field takes (default `false`); `resolveSpringLoadCapability(value)` → config or `null` (disabled). The capability field on `TilingWorkspacesCapability`, the set-mode wrapper wiring (hover → `HOVER`, scheduler timer → `TICK`, `fired` → `moveLeafToWorkspace` + `switchWorkspace` + `onWorkspaceSwitch({ via: "spring-load" })` + `onMoveLeaf`, then `REARM`) and the `TilingWorkspaceTabDragHover` end-to-end test are the **follow-up** after the pool-aware wrapper (H6) merges; the contract is the module doc of `engine/workspace-spring-load.ts`.
