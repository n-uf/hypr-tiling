...
| I3 | Ghost seated rect == target seat rect (no drift) | seat-rect effect measures the reservation rect; overlay portal (default body) removes ancestor-frame drift | NOT directly tested (DOM measurement) |
...
because it is an overlay portal (default `document.body`; redirect via
`overlayPortalContainer`) OUTSIDE the React root container's event-delegation
scope and outside every measurement scope; moving the real node there would
also make the pane's `[data-leaf-id]` disappear from the tree while the seat
needs it (`cc23956`). Cost: a host whose content is expensive renders it
twice during a drag (unchanged from slot mode).
